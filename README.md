...
Интеграция Telegram подготовлена, но отправка выключена до настройки серверного API. Инструкция: [docs/telegram.md](docs/telegram.md).
...
- **Форма заявки:** разместить серверный обработчик `functions/api/lead.js`, задать секреты на сервере и заполнить `data-endpoint` на пяти страницах. GitHub Pages серверный код не выполняет; токен нельзя помещать в браузер.
